...
POST /investigation/{investigation_id}/logs
  ...
  response: {
    ...
    "analysis": {
      ...
      "established_by_logs": "...", "established_by_investigation": "...",
      "evidence_gap": { "gap_detected": false, "categories": [], "source_hints": [], "signals": [] },
...
`analysis.evidence_gap` is a mechanical, text-based assessment (see
`context/evidence_gap.py`) of what kind of additional evidence the
EXISTING RCA's own wording suggests (never a metric threshold, never a
root-cause conclusion) - it's also used server-side to prioritize which
discovered log source to prefer when a resource has more than one match.
"Investigate Logs" is never blocked by `gap_detected` being `false` - it
remains a fully optional, user-triggered action; the assessment only
changes what gets prioritized once triggered.
`analysis.established_by_investigation` is a deterministic, code-computed
echo of the ORIGINAL metric-based RCA's own root_cause/summary (never
regenerated by Gemini); `analysis.established_by_logs` is Gemini's own
statement of what the NEW log evidence specifically adds - the dashboard
shows both side by side so a user can see exactly what came from where.
